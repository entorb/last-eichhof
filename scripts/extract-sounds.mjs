// Extract original DOS sounds from beer_exe/BEER.DAT into OGG Vorbis.
...
const SCALE_4 = [
  0, 1, 2, 3, 4, 5, 6, 7, 0, -1, -2, -3, -4, -5, -6, -7, 1, 3, 5, 7, 9, 11, 13,
  15, -1, -3, -5, -7, -9, -11, -13, -15, 2, 6, 10, 14, 18, 22, 26, 30, -2, -6,
  -10, -14, -18, -22, -26, -30, 4, 12, 20, 28, 36, 44, 52, 60, -4, -12, -20,
  -28, -36, -44, -52, -60,
];
const ADJUST_4 = [
  0, 0, 0, 0, 0, 16, 16, 16, 0, 0, 0, 0, 0, 16, 16, 16, 240, 0, 0, 0, 0, 16, 16,
  16, 240, 0, 0, 0, 0, 16, 16, 16, 240, 0, 0, 0, 0, 16, 16, 16, 240, 0, 0, 0, 0,
  16, 16, 16, 240, 0, 0, 0, 0, 0, 0, 0, 240, 0, 0, 0, 0, 0, 0, 0,
];
...
function levelUsage(files, level) {
  ...
  if (tbl) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (foe === 0x8002) add(x, `attack@${time}`);
    }
  }
    ...
    for (const at of libPointers(foe)) {
      ...
      while (p + 2 <= foe.length) {
        ...
        if (w === 0x8001) p += 4;
          ...
          add(foe.readInt16LE(p + 2), `foe:spr${sprite}`);
          p += 4;
        } else p += 4;
      }
    }
    ...
    libPointers(exp).forEach((at, i) => {
      ...
      while (p + 2 <= exp.length) {
        ...
        if (w === 0x8001) p += 6;
          ...
          add(exp.readInt16LE(p + 2), `expl#${i}`);
          p += 4;
...
for (const stale of REMOVE_STALE)
  rmSync(resolve(OUT_DIR, stale), { force: true });
...
for (let level = 0; level < 5; level++) {
  ...
  levelCues[webLevel] = {
    explosion:
      cues.explosion === null ? null : `level${webLevel}-snd${cues.explosion}`,
    bossExplosion:
      cues.bossExplosion === null
        ? null
        : `level${webLevel}-snd${cues.bossExplosion}`,
    enemyShot:
      cues.enemyShot === null ? null : `level${webLevel}-snd${cues.enemyShot}`,
...
const ts = `// Generated by scripts/extract-sounds.mjs from beer_exe/BEER.DAT.
...
console.log(
  `wrote ${sounds.length} OGGs + audition.html to public/assets/sounds`,
);
