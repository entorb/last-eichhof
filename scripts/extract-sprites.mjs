...
function shieldFor(role, shield) {
  if (role === "chaff") return Math.max(1, Math.min(6, shield));
  if (role === "miniboss") return Math.max(12, Math.min(40, shield));
  return 1;
}
...
for (let level = 0; level < LEVELS; level++) {
  ...
  for (const fi of [...valid].sort((a, b) => a - b)) {
    ...
    const spec = {
      texture,
      shield: shieldFor(role, foe.shield),
      score: clamp(foe.score, 50, 5000),
      role,
  ...
  for (const e of tbl) {
    ...
    if (kinds.get(kind).role === "boss") {
      ...
      maxBossShield = Math.max(maxBossShield, foes[e.foe].shield);
    }
  }
  const checkpoints = marks
    .filter((t) => t < firstBossAt)
    .sort((a, b) => a - b);
  const bossHp = clamp(Math.round(maxBossShield / 4), 15, 120);
