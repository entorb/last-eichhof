...
export function resolveTouchControls(mode: ControlsMode, deviceTouch: boolean): boolean {
...
export function stickFromForce(fx: number, fy: number, radius: number, threshold = 0.25): Stick {
...
export function runControlsSelfCheck(): void {
  ...
  assert(Math.abs(Math.hypot(diag.x, diag.y) - 1) < 1e-9, "diagonal normalized")
