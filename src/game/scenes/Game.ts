...
import { endRun, getRun, hasRun, newRun, type Run, shipSpeed } from "../data/run"
...
import { moveVector, resolveTouchControls, stickFromForce } from "../input/controls"
...
export class Game extends Scene {
  ...
  create() {
    ...
    if (!hasRun()) newRun()
    this.run = getRun()
    this.bg = this.add.rectangle(0, 0, PLAY.w, PLAY.h, 0x05060d).setOrigin(0).setDepth(-20)
  ...
  private buildTouchControls() {
    ...
    zone.setInteractive(new Geom.Rectangle(-2000, -2000, 4000, 4000), Geom.Rectangle.Contains)
  ...
  private togglePause() {
    if (this.state !== "play" && this.state !== "shield" && this.state !== "dead") {
      return
    }
    this.joystick?.setVisible(false)
  ...
  private updatePauseButton() {
    ...
    const active = this.state === "play" || this.state === "shield" || this.state === "dead"
  ...
  private fire(dt: number) {
    ...
    for (const [i, { weapon, dx, dy }] of this.loadout.entries()) {
      ...
      for (const em of weapon.emitters) {
        this.shots.push(this.makeShot(this.ship.x + dx, this.ship.y + dy, weapon.tint, em))
      }
    }
  }
  ...
  private makeShot(x: number, y: number, tint: number, em: Emitter): Shot {
    ...
    return {
      ...
      tint,
      release: em.release ? { after: em.release.after, shots: em.release.shots } : undefined,
    }
  }
  ...
  private releaseShots(parent: Shot) {
    ...
    for (const em of rel.shots) {
      this.shots.push(this.makeShot(parent.sprite.x, parent.sprite.y, parent.tint, em))
    }
  }
  ...
  private expireEnemy(e: Enemy): boolean {
    ...
    const gone = p.y > BOTTOM + 120 || p.y < TOP - 400 || p.x < -PLAY.w || p.x > PLAY.w * 2
    if (gone && role !== "boss") {
      e.sprite.destroy()
      return true
    }
  ...
  private collideShotVsEnemies(shot: Shot, deadShots: Set<Shot>, deadEnemies: Set<Enemy>) {
    for (const e of this.enemies) {
  ...
  private afterDeath() {
    if (this.lives <= 0) {
      this.state = "gameover"
      this.message.setText(this.touch ? "GAME OVER\nTAP TO CONTINUE" : "GAME OVER\nPRESS SPACE")
      getSfx().gameOver()
  ...
  private countBosses(queue: RosterSpawn[], from: number, inclusive = false): number {
    return queue.filter(
      (s) => !s.cmd && FOES[s.kind].role === "boss" && (inclusive ? s.at >= from : s.at > from),
    ).length
  }
  ...
  private updateHud() {
    ...
    this.levelText.setText(`LEVEL ${this.run.level}  ${getLevel(this.run.level).name}`)
  }
