...
export class Shop extends Scene {
  ...
  create() {
    ...
    this.add.rectangle(0, 0, PLAY.w, PLAY.h, 0x05060d).setOrigin(0).setDepth(-20)
    this.add
  ...
  private clampGhost(v: number, center: number, size: number, sprite: number): number {
  ...
  private buildPlacement() {
    ...
    this.ghostX = this.clampGhost(PLACE.x - PLACE.w / 2 + MARGIN + w.w / 2, PLACE.x, PLACE.w, w.w)
    this.ghostY = this.clampGhost(PLACE.y - PLACE.h / 2 + MARGIN + w.h / 2, PLACE.y, PLACE.h, w.h)

    const box = this.add.graphics().setDepth(5)
    box.lineStyle(2, 0x334155)
    box.strokeRect(PLACE.x - PLACE.w / 2, PLACE.y - PLACE.h / 2, PLACE.w, PLACE.h)
    box.fillStyle(0x1e293b, 0.4)
    box.fillRect(PLACE.x - PLACE.w / 2, PLACE.y - PLACE.h / 2, PLACE.w, PLACE.h)
    ...
    if (this.touch) {
      this.ghost.setInteractive({
        hitArea: new Geom.Rectangle(-w.w / 2 - 24, -w.h / 2 - 24, w.w + 48, w.h + 48),
        hitAreaCallback: Geom.Rectangle.Contains,
        draggable: true,
        useHandCursor: true,
      })
      this.ghost.on("drag", (_pointer: Input.Pointer, dragX: number, dragY: number) => {
  ...
  private refresh() {
    ...
    this.weaponInfo.setText(`DIRECTION  ${dir}\nDAMAGE  ${dmg}`).setVisible(true)
  }
