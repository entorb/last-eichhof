...
export class PathRunner {
  ...
  private stepWait(s: Extract<CmdState, { kind: "wait" }>, budget: number): number {
  ...
  private stepHome(s: Extract<CmdState, { kind: "home" }>, budget: number, target?: Vec): number {
    const use = Math.min(budget, s.remaining / 1000)
    if (target) {
  ...
  private stepLine(s: Extract<CmdState, { kind: "line" }>, budget: number): number {
...
export function runPathSelfCheck(): void {
  ...
  assert(Math.abs(events.pos.x - 10) < 0.001, `loop resets to mark x=${events.pos.x}`)
  assert(Math.abs(events.pos.y) < 0.001, `loop resets to mark y=${events.pos.y}`)
}
