...
describe("levels", () => {
  ...
  it("never schedules an invincible or transparent boss", () => {
    for (const level of LEVELS) {
      for (const spawn of level.build()) {
        ...
        expect(foe.transparent ?? false, `${spawn.kind} transparent`).toBe(false)
      }
    }
  })
  ...
  it("counts every scheduled boss and spawns it after t=0", () => {
    for (const level of LEVELS) {
      const bosses = level.build().filter((s) => !s.cmd && FOES[s.kind].role === "boss")
  ...
  it("marks DOS FOE_STOPCOUNT foes as scheduled killable minibosses", () => {
    ...
    for (const [kind, foe] of stops) {
      ...
      expect(scheduled.has(kind as FoeKind), `${kind} not scheduled`).toBe(true)
    }
  })
  ...
  it("carries the DOS .DSC bonus score and money", () => {
    expect(LEVELS.map((l) => l.bonusScore)).toEqual([0, 20000, 20000, 50000, 20000])
