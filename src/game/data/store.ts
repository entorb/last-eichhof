...
export function addResult(results: GameResult[], result: GameResult): GameResult[] {
...
export function sortResults(results: GameResult[], sort: ResultSort): GameResult[] {
  return [...results].sort(
    sort === "points" ? (a, b) => b.score - a.score || a.at - b.at : (a, b) => b.at - a.at,
  )
}

export function clampScroll(scroll: number, total: number, rows: number): number {
...
export function parseSettings(raw: string | null): Settings {
  ...
  const controls =
    obj.controls === "auto" || obj.controls === "touch" || obj.controls === "keyboard"
      ? obj.controls
...
export function runStoreSelfCheck(): void {
  ...
  assert(byPoints[0]?.score === 30 && byPoints[1]?.score === 20, "points sort order")
  ...
  assert(sortResults([tie1, tie2], "points")[0] === tie2, "tie breaks to older result")
  ...
  assert(
    parseSettings(JSON.stringify({ difficulty: "nope" })).difficulty === "normal",
    "invalid difficulty",
  )
  ...
  assert(parseSettings(JSON.stringify({ autoFire: true })).autoFire === true, "valid auto-fire")
  assert(parseSettings(JSON.stringify({ autoFire: "yes" })).autoFire === false, "invalid auto-fire")
