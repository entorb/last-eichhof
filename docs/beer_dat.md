...
Game downloaded from <https://archive.org/download/TheLastEichhof/beer11.zip> into `beer_exe/`.

Reference for decoding the original DOS assets bundled in `beer_exe/BEER.DAT`
(`ALPHA-HELIX COMBINER VER 3.3` archive). Source of truth: `beer_src/COMBINER.C`,
`beer_src/FILEMAN.C`, `beer_src/SLIB.C`, `beer_src/XMODEC.C`, `beer_src/XMODE.H`.
...
Layout (`beer_src/SLIB.C` `readslib`/`saveslib`):
...
At `2 + off` sits `struct sprstrc` (`beer_src/XMODE.H`):
...
array in `beer_src/XMODEC.C` (`setstandardpalette()` copies it into `palette`
...
`beer_src/BALLER.H`): `{ flags u16, shield u16, score u16, expl u16,
...
The rewrite now mirrors the DOS rosters 1:1 (web level *n* = DOS `LEVEL{n-1}`).
...
- `src/game/data/foeRosters.ts` — `FoeKind` union, `FOES` (`texture`, `shield`,
  ...
  (per-level `name`, `spawns`, `checkpoints`, `bosses`, `bossHp`).
  ...
  - role: `boss` if `ENDLEVEL || score < 0`, else `miniboss` if `shield >= 15`,
    else `chaff`. Shields are clamped by role; scores clamped to `[50, 5000]`.
...
`scripts/enemy-map.json` now only holds `scale` and the fixed `extras`
(`ship`, `explosion`). Re-run `node scripts/extract-sprites.mjs` after changing
it or the DOS data.
